    ...
    for(int n = frozen_c; n < 2 * doccpi; ++n){
        for(int N = 0; N < frozen_c; ++N){
            ...
            for(int a = 2 * doccpi; a < nso - frozen_v; ++a){
                for(int b = 2 * doccpi; b < nso - frozen_v; ++b){
                    for(int j = frozen_c; j < 2 * doccpi; ++j){
                        temp1 += so_ints[four_idx(N, j, a, b, nso)] * amp_t[four_idx(n, j, a, b, nso)];        
                    }
                }
            }
    ...
    for(int d = 2 * doccpi; d < nso - frozen_v; ++d){
        for(int D = nso - frozen_v; D < nso; ++D){
            ...
            for(int a = 2 * doccpi; a < nso - frozen_v; ++a){
                for(int i = frozen_c; i < 2 * doccpi; ++i){
                    for(int j = frozen_c; j < 2 * doccpi; ++j){
                        temp1 += so_ints[four_idx(i, j, a, D, nso)] * amp_t[four_idx(i, j, a, d, nso)];        
                    }
                }
            }
        ...
        for(int A = nso - frozen_v; A < nso; ++A){
            ...
            for(int i = frozen_c; i < 2 * doccpi; ++i){
                for(int j = frozen_c; j < 2 * doccpi; ++j){
                    for(int a = 2 * doccpi; a < nso - frozen_v; ++a){
                        for(int b = 2 * doccpi; b < nso - frozen_v; ++b){
                            temp2 -= ( so_ints[four_idx(i, I, i, A, nso)] - so_ints[four_idx(a, I, a, A, nso)] ) * amp_t[four_idx(i, j, a, b, nso)] * amp_t[four_idx(i, j, a, b, nso)];
                        }
                    }
                }
            } 

            Z_temp->set(0, I, A, (temp1 + temp2) / ( epsilon[I] - epsilon[A] ));
    ...
    for(int c = 2 * doccpi ; c < nso - frozen_v; ++c){
        for(int N = 0; N < frozen_c; ++N){
            ...
            for(int i = frozen_c; i < 2 * doccpi; ++i){
                for(int j = frozen_c; j < 2 * doccpi; ++j){
                    for(int b = 2 * doccpi; b < nso - frozen_v; ++b){
                        temp2 -= so_ints[four_idx(i, j, N, b, nso)] * amp_t[four_idx(i, j, c, b, nso)];
                    }
                }
            }
            ...
            for(int i = frozen_c; i < 2 * doccpi; ++i){
                for(int j = frozen_c; j < 2 * doccpi; ++j){
                    for(int a = 2 * doccpi; a < nso - frozen_v; ++a){
                        for(int b = 2 * doccpi; b < nso - frozen_v; ++b){
                            temp3 -= ( so_ints[four_idx(i, N, i, c, nso)] - so_ints[four_idx(a, N, a, c, nso)] ) * amp_t[four_idx(i, j, a, b, nso)] * amp_t[four_idx(i, j, a, b, nso)];
                        }
                    }
                }
            } 

            Z_temp->set(0, N, c, (temp1 + temp2 + temp3) / ( epsilon[N] - epsilon[c] ));
            Z_temp->set(0, c, N, Z_temp->get(0, N, c));
        }
    }   
    ...
    for(int C = nso - frozen_v ; C < nso; ++C){
        for(int n = frozen_c; n < 2 * doccpi; ++n){
            ...
            for(int j = frozen_c; j < 2 * doccpi; ++j){
                for(int a = 2 * doccpi; a < nso - frozen_v; ++a){
                    for(int b = 2 * doccpi; b < nso - frozen_v; ++b){
                        temp2 += so_ints[four_idx(C, j, a, b, nso)] * amp_t[four_idx(n, j, a, b, nso)];
                    }
                }
            }
            ...
            for(int i = frozen_c; i < 2 * doccpi; ++i){
                for(int j = frozen_c; j < 2 * doccpi; ++j){
                    for(int a = 2 * doccpi; a < nso - frozen_v; ++a){
                        for(int b = 2 * doccpi; b < nso - frozen_v; ++b){
                            temp3 += ( so_ints[four_idx(a, n, a, C, nso)] - so_ints[four_idx(i, n, i, C, nso)] ) * amp_t[four_idx(i, j, a, b, nso)] * amp_t[four_idx(i, j, a, b, nso)];
                        }
                    }
                }
            }    
...
Z_MP2->print();
    ...
    for(int p = 0; p < nso; ++p){
        for(int q = 0; q < nso; ++q){
            ...
            dipole_MP2 += 0.5*Z_MP2->get(0, p, q) * Dp_mo->get(0, p/2, q/2);
